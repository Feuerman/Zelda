$(document).ready(function() {
	...
	$('.close-panel, .desc__button, .configurator-product__item').on('click', function(event) {
		togglePanelConf();
	});	

	function togglePanelConf() {
		$('.configurator__bottom-panel').toggleClass('active-panel').css('position', 'absolute');
		$('.configurator__bottom-panel').bind("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd", function(){
			adaptiveHeightConf();
		});
	};
	...
	$('.js-rating-change span').on('click', function(){
		...
		changeRating(currentElIndex, 'changed');
	});

	function changeRating(indexChange, changed) {
		$('.js-rating-change span').each(function (index) {
			if(indexChange >= index) {				
				changed ? $(this).addClass('selected') : '';
				$(this).addClass('active');
			} else {				
				changed ? $(this).removeClass('selected') : '';
